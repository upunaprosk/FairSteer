...
import numpy as np
from utils import get_llama_activations_bau, tokenized_bbqa
import argparse
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='bbqa')
    ...
    dataset = load_dataset('json', data_files=f'generate_data/BBQA.json')['train']
    prompts, labels, _, positions, _, _, _ = tokenized_bbqa(dataset, tokenizer)
    ...
    np.save(f'activations/{args.dataset}/{args.model_name}/layer_wise.npy',
            all_layer_wise_activations)

    print("Saving mlp wise activations")
    np.save(f'activations/{args.dataset}/{args.model_name}/mlp_wise.npy',
            all_mlp_wise_activations)
