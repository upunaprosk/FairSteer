...
class CrowSPairsRunner:
    ...
    def __init__(
        ...
        self._intervention_fn = intervention_fn
    ...
    def _likelihood_score(self):
        ...
        results = {"Total examples": N, "Metric score": round((stereo_score + antistereo_score) / N * 100, 2),
                   "Stereotype score": round(stereo_score / total_stereo * 100, 2),
                   "Anti-stereotype score": 0,
                   "Neutral": round(neutral / N * 100, 2)}
        ...
        if antistereo_score != 0:
            ...
            results["Anti-stereotype score"] = round(antistereo_score / total_antistereo * 100, 2)
    ...
    def _joint_log_probability(self, tokens):
        ...
        # start_token = (
        #     torch.tensor(self._tokenizer.encode("<s>", add_special_tokens=False))
        #     .to(self._model.device)
        #     .unsqueeze(0)
        # )
        # start_token = (
        #     torch.tensor(self._tokenizer.encode("<|begin_of_text|>", add_special_tokens=False))
        #     .to(device)
        #     .unsqueeze(0)
        # )
        # start_token = (
        #     torch.tensor(self._tokenizer.encode("<bos>", add_special_tokens=False))
        #     .to(device)
        #     .unsqueeze(0)
        # )
        ...
        tokens_tensor = torch.tensor(tokens).to(self._model.device).unsqueeze(0)
        ...
        with torch.no_grad():
            if self._is_self_debias:
                ...
                if self._interventions == None:
                    intervene = id
                    layers_to_intervene = []
                else:
                    intervene = partial(self._intervention_fn, start_edit_location=start_edit_location)
                    layers_to_intervene = list(self._interventions.keys())
                ...
                if self._baseline_mode:
                    ...
                    with TraceDict(self._model, layers_to_intervene, edit_output=intervene) as ret:
                        output = torch.softmax(self._model(tokens_tensor)[0], dim=-1)
